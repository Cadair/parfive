...
def test_ftp_login_anon(tmpdir):
    ...
    dl = Downloader()
    dl.enqueue_file(
        "ftp://ftp.ngdc.noaa.gov/STP/swpc_products/daily_reports/solar_region_summaries/2002/04/20020414SRS.txt", path=tmpdir)
    assert dl.queued_downloads == 1
    f = dl.download()
    assert len(f) == 1
...
@skip_windows
@pytest.mark.allow_hosts(True)
def test_ftp_login_anon_in_url(tmpdir):
    tmpdir = str(tmpdir)
    dl = Downloader()
    dl.enqueue_file(
        "ftp://anonymous:anonymous@ftp.ngdc.noaa.gov/STP/swpc_products/daily_reports/solar_region_summaries/2002/04/20020414SRS.txt", path=tmpdir)
    assert dl.queued_downloads == 1
    f = dl.download()
    assert len(f) == 1
    assert len(f.errors) == 0
